...
export class ProductsService {
  ...
  // TODO: Paginar
  async findAll(paginationDto: PaginationsDto) {
  ...
  async findOne(termino: string) {
    ...
    if (isUUID(termino)) {
      ...
      product = await queryBuilder
        .where('UPPER(title) =UPPER(:title) or slug =:slug', {
    ...
    if (!product)
      throw new NotFoundException(`Product with id ${termino} not found`);
  ...
  private handleExceptions(error: any) {
    ...
    throw new InternalServerErrorException(
      'Unexpected error, chech server logs',
    );
  }
